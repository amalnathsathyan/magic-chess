...
fn test_initialize_match() {
    ...
    assert!(pk_eq(&mint.to_bytes(), &chess_match.betting_token_mint.to_bytes()));
...
fn test_join_match() {
    ...
    assert!(pk_eq(&p2.pubkey().to_bytes(), &chess_match.players[1].to_bytes()));
...
fn test_abort_match() {
    ...
    let abort_ix = abort_match_ix(
        &chess_match_pda, &escrow_pda, &p1_ata, &p1_pk,
    );
